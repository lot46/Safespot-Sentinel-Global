...
export async function registerPlugins(app: FastifyInstance): Promise<void> {
  // Expose config on Fastify instance for typed access
  ...
  await app.register(cors, {
    ...
    allowedHeaders: ['Origin','X-Requested-With','Content-Type','Accept','Authorization','X-Request-ID','X-API-Key','X-CSRF-Token'],
  ...
  await app.register(rateLimit, {
    ...
    skipOnError: true,
    keyGenerator: (request: any) => (request.headers['x-forwarded-for'] as string) || request.ip,
  ...
  // JWT plugin (used only for request decoration in some routes)
  const fastifyJwt = (await import('@fastify/jwt')).default as any;
  await app.register(fastifyJwt, {
    secret: config.security.jwt.secret,
    sign: { algorithm: 'HS256', expiresIn: config.security.jwt.accessExpiresIn },
  ...
  if (config.app.isDevelopment) {
    ...
    await app.register(fastifySwagger, {
      openapi: {
        openapi: '3.0.0',
        info: { title: 'SafeSpot Sentinel Global API', version: config.app.version },
  ...
  // Add a generic authenticate decorator fallback (ensures existence for TS and tests)
